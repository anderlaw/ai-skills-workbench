        ...
        <CardHeader className="grid gap-4">
          ...
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <label className="relative block">
              <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-content-muted" size={16} />
              <Input
                value={keyword}
      ...
      <SideDrawer title="节点详情" description={getNodePath(node, nodeById)} onClose={onClose}>
        <div className="grid gap-5">
          <div className="rounded-lg border border-line bg-surface p-4">
            <div className="flex flex-wrap items-center gap-2">
              <NodeIcon nodeType={node.nodeType} className="text-accent-strong" />
              <span className="font-semibold text-content-strong">{node.name}</span>
  ...
  return (
    <form className="grid gap-4" onSubmit={handleSubmit}>
      ...
      <div className="rounded-lg border border-line bg-surface-muted p-3 text-sm">
        <div className="text-xs font-semibold text-content-muted">父级</div>
        <div className="mt-1 font-medium text-content-strong">{parentPath}</div>
      </div>

      <Field label="节点类型">
        <div className="flex h-10 items-center gap-2 rounded-lg border border-line bg-surface-muted px-3 text-sm text-content">
          <NodeIcon nodeType={nodeType} className="text-accent-strong" />
    ...
    <div>
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <NodeIcon nodeType={node.nodeType} className={inactive ? "text-content-muted" : "text-accent-strong"} />
        </span>
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-3">
      ...
      <span className="break-all text-content">{value}</span>
    </div>
  );
}
...
function ErrorBox({ error, className }: { error: unknown; className?: string }) {
  return (
    <div className={clsx("rounded-lg border border-danger-line bg-danger-muted px-4 py-3 text-sm font-medium text-danger", className)}>
      {errorMessage(error)}
    </div>
  );
}
